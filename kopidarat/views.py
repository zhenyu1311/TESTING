...
#             recommended_activities = cursor.fetchall()
        ...
        #grouping_sql = " GROUP BY a.activity_id, u.full_name, a.price, a.start_point, a.start_date_time, a.destination, count_passenger.count, a.capacity"
...
#                     price_filters += " OR a.price="+"'"+price+"'"
#                 price_filter_sql = " AND("+price_filters[3:]+")"
...
#             with connection.cursor() as cursor:
#                 cursor.execute(all_activities_sql + price_filter_sql + time_filter_sql + ordering_sql)
#                 activities = cursor.fetchall()
     

#             #with connection.cursor() as cursor:
#                 #cursor.execute(all_activities_sql+display_date_sql+price_filters+
...
def passenger(request, activity_id):
    ...
    Return:
        render function: render the passenger page (path: /passenger/<activity_id>)
    """
    ...
    if user_type == 'administrator' and user_email is not False:
        ...
        if request.method == 'POST':
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute('INSERT INTO activity (driver,start_point,price,start_date_time,destination,capacity) VALUES (%s,%s,%s,%s,%s,%s)', [
                    request.session.get(
                        "email"), request.POST['start_point'], request.POST['price'], request.POST['start_date_time'],
                    request.POST['destination'], request.POST['capacity']
                ])
                # Get the activity details
                cursor.execute('SELECT activity_id FROM activity WHERE driver =  %s AND start_point = %s AND price = %s AND start_date_time = %s AND destination = %s AND capacity = %s', [
                    request.session.get(
                        "email"), request.POST['start_point'], request.POST['price'], request.POST['start_date_time'],
                    request.POST['destination'], request.POST['capacity']
                ])
    ...
    if user_type == 'administrator' and user_email is not False:
        ...
        if request.method == 'POST':
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute('UPDATE activity SET price = %s, start_point = %s, start_date_time = %s, destination = %s, capacity = %s WHERE activity_id = %s', [
                    request.POST['price'], request.POST['start_point'], request.POST[
                        'start_date_time'], request.POST['destination'], request.POST['capacity'], activity_id
                ])
                cursor.execute('SELECT * FROM activity WHERE activity_id=%s', [activity_id])
                this_activity = cursor.fetchone()
...
# View function for price popularity list
...
def frontpage(request):
    ...
    Argument:
        request: HTTP request
    Return:
        render function: renders the price_popularity page (path: /price_popularity)
    '''
    user_email = request.session.get("email", False)
    if user_email is not False:
        return HttpResponseRedirect(reverse('index'))
...
#         cursor.execute(
#             'SELECT price, COUNT(*) FROM activity GROUP BY price ORDER BY COUNT(*) DESC')
#         categories = cursor.fetchall()
